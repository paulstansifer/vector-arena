...
```
src/
├── main.rs                   # App setup, system registration, world startup
├── lib.rs                    # Module exports, GameLayer enum, WorldBounds, AGENT_RADIUS
├── player.rs                 # Player component, movement, click-to-move
├── monster.rs                # Monster marker component
├── nav.rs                    # Landmass→Avian2D velocity bridge for monsters
├── fov.rs                    # FOV raycasting, exploration tracking, mesh overlays
├── item.rs                   # Items, inventory, pickup animation
├── dungeon/
│   ├── mod.rs                # Re-exports
...
└── effects/
    ├── mod.rs                # Re-exports
    ├── projectile.rs         # Magic missiles, trails, knockback, bullet-time
...
3. Spawn: player (random room), 2 monsters (other rooms), 4–5 items
4. Spawn: doors with revolute-joint hinges, FOV overlay meshes, camera
...
**New item type** — add a variant to `ItemKind` in [src/item.rs](src/item.rs), spawn it in `main.rs` with a mesh/material.

**New effect** — add a file under `src/effects/`, register its plugin/systems in `main.rs`, add it to the `effects/mod.rs` re-exports.

**New monster behavior** — extend `monster.rs` with components, add systems in `main.rs` or a new file.
