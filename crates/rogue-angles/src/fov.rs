...
pub fn update_fov(
    ...
    // A vision multiplier away from its 1.0 resting value means some game-side effect is
    // actively shrinking/growing the FOV radius (e.g. it's ramping down); keep recomputing
    // every frame while that's true, same as the transform-changed check below.
    let modifiers_active = viewer_modifiers.map(|m| m.vision_multiplier != 1.0).unwrap_or(false);
    if !viewer_transform.is_changed() && !modifiers_active && fov_mesh_handle.0.path().is_some() {
        return;
    }
